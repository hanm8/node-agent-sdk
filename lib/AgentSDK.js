...
class SDK extends Events {
    constructor(conf) {
        ...
        this.refreshSessionInterval = conf.refreshSessionInterval || (60000 * 1);//10 min
        this.conf = conf;
    ...
    _reconnect (delay = reconnectInterval, attempt = 1) {
    ...
    _refreshSession(err, data) {
        ...
        this.csdsClient.getAll((csdsErr, domains) => {
            if (!err && !csdsErr && (!data || !data.error) && this.csrf && domains) {
                ...
                this.reconnect();
                console.log('could not refresh token, username:' + this.conf.username)
